...
# The export CLI, spawned like any other builder. It used to be run straight out of
# packages/implicitjs/scripts, which made it the one Node entry that was not bundled --
# and an unbundled entry needs its whole dependency graph at runtime, so every skill that
# exported an implicit model had to vendor all of implicitjs beside it. The thin wrapper
# in packages/cadjs/bin is esbuilt self-contained per skill runtime; the CLI itself still
# lives in implicitjs and is still its own `npm run export`.
...
def implicit_export_cli() -> Path:
    ...
    Resolved exactly like every other builder, through :func:`node_builder_script`, so the
    dev checkout gets the source under ``packages/cadjs/bin`` and a skill runtime gets its
    own esbuilt copy. Missing is a PACKAGING failure and says which path it looked at.
    """
